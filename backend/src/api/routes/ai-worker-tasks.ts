...
/**
 * POST /api/v1/ai-worker-tasks/:id/trigger
 * Immediately trigger task processing (push-based execution)
 *
 * This endpoint accepts internal service key auth to allow immediate task execution
 * without waiting for orchestrator polling. Called after enqueueing to SQS for push-based workflow.
 *
 * MUST be defined BEFORE router.use(authenticateRequest) to bypass normal auth.
 */
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info("Triggering immediate task processing (push-based)", { taskId: id });
      ...
      // Only trigger if task is in queued state
      if (task.status !== "queued") {
        logger.info("Task not in queued state, skipping trigger", {
          taskId: id,
          status: task.status,
        });
        return res.json({
          success: true,
          message: "Task not in queued state",
          taskId: id,
          status: task.status,
        });
      }

      // Import dependencies for spawning worker task
      const { getECSTaskRunner } = await import("../../shared/services/ecs-task-runner");
      const { AIWorkerInstance } = await import("../../shared/models/AIWorkerInstance");

      const workerRepo = dataSource.getRepository(AIWorkerInstance);

      // Check persona slot availability (same as orchestrator)
      ...
      if (activeTaskForPersona) {
        logger.info("Persona slot occupied, task will be processed by orchestrator poll", {
          taskId: task.id,
        ...
        return res.json({
          success: true,
          message: "Persona slot occupied, will retry via orchestrator",
          taskId: id,
        });
      }
      ...
      if (claimResult.affected === 0) {
        logger.info("Task already claimed by another process", { taskId: id });
        return res.json({
          success: true,
      ...
      logger.info("Task triggered immediately (push-based)", {
        taskId: id,
        ecsTaskId,
        persona: task.workerPersona,
      });
